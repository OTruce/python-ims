...
def update_item(item_id):
    """
    Partially update an existing item. Only fields present in the
    JSON body get changed; everything else stays the same.
    """
    item = find_item_by_id(item_id)
    if item is None:
        return jsonify({"error": f"Item {item_id} not found"}), 404
...
def fetch_item_from_external(barcode):
    """
    Look up a barcode on OpenFoodFacts. If found, add it to our
    inventory database and return it. If not found, 404.
    """
